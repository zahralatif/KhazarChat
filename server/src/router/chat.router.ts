...
// Get a chat by ID
router.get("/:id", ClerkExpressRequireAuth(), async (req: Request, res: Response, next: NextFunction) => {
    const userId = req.auth?.userId;
...
router.put("/:id", ClerkExpressRequireAuth(), async (req: Request, res: Response, next: NextFunction) => {
    const userId = req.auth?.userId;
    const chatId = req.params.id;
